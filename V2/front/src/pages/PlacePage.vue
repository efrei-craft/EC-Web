<template>
  <div class="w-screen h-screen flex flex-col items-center bg-gradient-to-b from-[#0a0a14] to-[#131330] overflow-hidden p-2">
    <div class="bg-white/5 backdrop-blur-md rounded-xl shadow-lg border border-white/10 w-full max-w-[98vw] h-full max-h-[98vh] flex flex-col">
      ...
      <!-- Positioned container for absolute tooltip -->
...
const view = ref({
  scale: 1,
  minScale: 1,      // do not go below the base fit
  maxScale: 12,     // allow deep zoom
  panX: 0,
  panY: 0
})
...
function setupCanvas() {
  ...
  // When canvas size changes, reset view to base fit
  view.value.scale = 1
  view.value.panX = 0
  view.value.panY = 0
}
...
function render() {
  ...
  // Apply view transform, then draw
...
// Map canvas event offsets -> world (unscaled) coords
function toWorld(e: MouseEvent) {
  const mx = (e as MouseEvent).offsetX
  const my = (e as MouseEvent).offsetY
...
// Cursor-centered zoom (wheel)
function handleWheel(e: WheelEvent) {
  if (!canvas.value) return
  const factor = e.deltaY < 0 ? 1.15 : 1 / 1.15
  // Trackpad ctrlKey reversal (optional)
  ...
  // Pan so the same world point stays under cursor
...
// Tooltip on click with transforms handled
async function handleCanvasClick(e: MouseEvent) {
  ...
  // Screen position from world (respecting pan/zoom)
